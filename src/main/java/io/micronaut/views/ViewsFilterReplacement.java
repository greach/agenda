...
@Replaces(ViewsFilter.class)
@Filter("/**")
public class ViewsFilterReplacement implements HttpServerFilter {

    private static final Logger LOG = LoggerFactory.getLogger(ViewsFilter.class);

    protected final Integer order;
    protected final BeanLocator beanLocator;
    private final Collection<ModelDecorator> modelDecorators;
    /**
     * Constructor.
     *
     ...
     */
    public ViewsFilterReplacement(BeanLocator beanLocator,
                                  @Nullable ViewsFilterOrderProvider viewsFilterOrderProvider,
                                  Collection<ModelDecorator> modelDecorators) {
                        ...
                        if (optionalView.isPresent()) {
                            ...
                            if (optionalViewsRenderer.isPresent()) {
                                ...
                                if (viewsRenderer.exists(view)) {
                                    ...
                                    model.putAll(modelOf(resolveModel(body)));
                                    for (ModelDecorator modelDecorator : modelDecorators) {
